...
#SBATCH --output=logs/aria_1gpu_%j.out
#SBATCH --error=logs/aria_1gpu_%j.err

# aria dataset training pipeline - single 80gb a100
...
RUN_ROOT="${SCRATCH}/embodied-cot-aria/checkpoints/aria_1gpu"
ADAPTER_TMP="${SCRATCH}/embodied-cot-aria/adapter_tmp"
BBOX_LOG_DIR="${SCRATCH}/embodied-cot-aria/training_logs/bboxes/aria_1gpu"
EVAL_DIR="${SCRATCH}/embodied-cot-aria/evaluation/aria_1gpu"

mkdir -p $RUN_ROOT $ADAPTER_TMP $BBOX_LOG_DIR $EVAL_DIR

echo "=== verifying dataset ==="
python verify_dataset.py $DATA_ROOT

echo "=== training without gaze ==="
...
CHECKPOINT_NO_GAZE=$(find $RUN_DIR_NO_GAZE -maxdepth 1 -type d -name "openvla-7b*" | head -n 1)
CHECKPOINT_WITH_GAZE=$(find $RUN_DIR_WITH_GAZE -maxdepth 1 -type d -name "openvla-7b*" | head -n 1)

python scripts/visualize_inference.py \
    --test_json $TEST_JSON \
    --base_model "openvla/openvla-7b" \
    --finetuned_a $CHECKPOINT_WITH_GAZE \
    --finetuned_b $CHECKPOINT_NO_GAZE \
    --training_logs_dir $BBOX_LOG_WITH_GAZE \
    --output_dir $VIZ_DIR \
    --device cuda:0

echo "=== pipeline complete ==="
echo "check logs/aria_1gpu_*.out for full output"
