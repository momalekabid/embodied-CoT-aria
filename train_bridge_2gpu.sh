...
#SBATCH --output=logs/bridge_2gpu_%j.out
#SBATCH --error=logs/bridge_2gpu_%j.err

# bridge dataset training pipeline - dual 80gb a100
...
RUN_ROOT="${SCRATCH}/embodied-cot-aria/checkpoints/bridge_2gpu"
ADAPTER_TMP="${SCRATCH}/embodied-cot-aria/adapter_tmp"
EVAL_DIR="${SCRATCH}/embodied-cot-aria/evaluation/bridge_2gpu"

mkdir -p $RUN_ROOT $ADAPTER_TMP $EVAL_DIR

echo "=== starting training run ==="

# dual gpu training - optimized for 80gb a100
torchrun --standalone --nnodes 1 --nproc-per-node 2 vla-scripts/finetune.py \
  ...
  --wandb_project "bridge-2gpu-train" \
...
echo "check logs/bridge_2gpu_*.out for full output"
